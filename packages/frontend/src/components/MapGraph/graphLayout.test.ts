...
describe("graphLayout", () => {
  ...
  it("preserves the viewport while current-zone follow is active", () => {
    expect(layoutOptionsForViewportMode(false)).toMatchObject({ fit: true });
    expect(layoutOptionsForViewportMode(true)).toMatchObject({ fit: false });
  });
});
